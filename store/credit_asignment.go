...
func (repo Repository) CreditAssignment(investment int32) (int32, int32, int32, error) {
	a, b, c, err := creditAssignment(investment)
	if err != nil {
		sql := repo.db.Exec("INSERT INTO credit_asignacion(monto,exito)VALUES(?,?)", investment, false)
		if sql.Error != nil {
			return 0, 0, 0, sql.Error
		}
		return 0, 0, 0, err
	}
	sql := repo.db.Exec("INSERT INTO credit_asignacion(monto,type_300,type_500,type_700,exito)VALUES(?,?,?,?,?)", investment, a, b, c, true)
	if sql.Error != nil {
		return 0, 0, 0, sql.Error
	}
	return a, b, c, nil
...
func creditAssignment(investment int32) (int32, int32, int32, error) {
